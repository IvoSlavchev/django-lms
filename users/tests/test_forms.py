...
class RegistrationFormTest(TestCase):
	...
	def test_if_valid_on_all_required_fields(self):
		form_data = {'username': 'example', 'email': 'example@example.com', 'is_teacher': False,
					 'password1': 'example', 'password2': 'example'}
		form = RegistrationForm(data=form_data)
		self.assertTrue(form.is_valid())

	def test_if_not_valid_on_different_passwords(self):
		form_data = {'username': 'example', 'email': 'example@example.com', 'is_teacher': True,
					 'password1': 'example', 'password2': 'example2'}
		form = RegistrationForm(data=form_data)
		self.assertFalse(form.is_valid())	

	def test_if_not_valid_on_empty_field(self):
		form_data = {'username': '', 'email': 'example@example.com', 'is_teacher': False,
					 'password1': 'example', 'password2': 'example'}
		form = RegistrationForm(data=form_data)
		self.assertFalse(form.is_valid())	
