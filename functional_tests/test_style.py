...
class StyleTest(FunctionalTest):
	...
	def test_correct_bootstrap_class(self):
		...
		elem_class = self.browser.find_element_by_link_text('Django-LMS').get_attribute("class")
		self.assertEqual(elem_class, 'navbar-brand')

		elem_class = self.browser.find_element_by_link_text('Log in').get_attribute("class")
